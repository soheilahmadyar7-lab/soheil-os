 ...
 *
 * Architecture:
 *   Hardware/TEE Android Keystore KEK -> wraps random 256-bit Vault DEK.
 *   Vault DEK exists only in RAM while the app is unlocked.
 *   All sensitive records use AES-256-GCM with a fresh 96-bit nonce.
 */
public final class SoheilCrypto {
    ...
    private static final String PREFIX = "S2.";
    ...
    public SoheilCrypto(Context context) {
        ...
        this.meta = this.context.getSharedPreferences(PREFS, Context.MODE_PRIVATE);
        ensureKek();
    }
    ...
    private void ensureKek() {
        try {
            ...
            throw new SecurityException("SOHEIL Keystore initialization failed", e);
        }
    }
    ...
    private void generateKek(boolean strongBox) throws Exception {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
            ...
            b.setUserAuthenticationValidityDurationSeconds(30);
        }
    ...
    /** Must only be called immediately after successful biometric/device authentication. */
    public synchronized void unlockVault() {
        try {
            SecretKey kek = getKek();
            String wrapped = meta.getString(WRAPPED_DEK, null);
            if (wrapped == null) {
                ...
                if (!meta.edit().putString(WRAPPED_DEK, encoded).commit()) {
                    ...
                    throw new SecurityException("Unable to persist wrapped vault key");
                }
                sessionDek = dek;
            } else {
            ...
            lockVault();
            throw new SecurityException("SOHEIL vault unlock failed", e);
        }
    }
    ...
    private SecretKey getKek() throws Exception {
        ...
        if (entry == null) throw new SecurityException("Vault KEK missing");
        return entry.getSecretKey();
    }

    private String wrapDek(SecretKey kek, byte[] dek) throws Exception {
        Cipher c = Cipher.getInstance("AES/GCM/NoPadding");
        c.init(Cipher.ENCRYPT_MODE, kek);
        byte[] ct = c.doFinal(dek);
        return b64(c.getIV()) + ":" + b64(ct);
    }

    private byte[] unwrapDek(SecretKey kek, String wrapped) throws Exception {
        String[] p = wrapped.split(":", -1);
        if (p.length != 2) throw new SecurityException("Invalid wrapped vault key");
        Cipher c = Cipher.getInstance("AES/GCM/NoPadding");
        c.init(Cipher.DECRYPT_MODE, kek, new GCMParameterSpec(GCM_TAG_BITS, fromB64(p[0])));
        byte[] dek = c.doFinal(fromB64(p[1]));
        if (dek.length != 32) {
            Arrays.fill(dek, (byte) 0);
    ...
    public synchronized String decrypt(String value) {
        ...
        try {
            ...
            SecretKeySpec key = new SecretKeySpec(sessionDek, "AES");
            Cipher c = Cipher.getInstance("AES/GCM/NoPadding");
            c.init(Cipher.DECRYPT_MODE, key, new GCMParameterSpec(GCM_TAG_BITS, fromB64(p[0])));
            byte[] pt = c.doFinal(fromB64(p[1]));
            String out = new String(pt, StandardCharsets.UTF_8);
            Arrays.fill(pt, (byte) 0);
            return out;
        } catch (Exception e) {
            throw new SecurityException("Vault integrity/authentication check failed", e);
        }
    }
